...
// 索引换成指标
link front = NULL; /* 队列的前端(head)   */
link rear = NULL;  /* 队列的后端(tail)   */
...
int enqueue(int value)
{
    ...
    if (rear == NULL)       /* 是否是第一次存入     */
        ...
        rear->next = new_node; /* 插入rear之后（尾挂）*/
...
int dequeue()
{
    ...
    {
        link top = front;       /* top暂存指向front    */
        front = front->next;    /* 删除之前节点，队头后移 */
        int temp = top->data;   /* 取出资料            */
        free(top);              /* 释回记忆体          */
        return temp;            /* 队列取出            */
    }
